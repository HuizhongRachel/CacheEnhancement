...
static char *vcache_opt;

/* victim cache hit latency (in cycles) */
static int vcache_lat;
	      ...
	      tick_t now)		/* time of access */
{
  ...
  if (vcache)
  {
	  /* access next level of data cache hierarchy */
	        lat = cache_access(vcache, cmd, baddr, NULL, bsize,
...
//*****************************************************************************
/* victim cache block miss handler function */
static unsigned int			/* latency of block access */
v_access_fn(enum mem_cmd cmd,		/* access cmd, Read or Write */
	      md_addr_t baddr,		/* block address to access */
	      int bsize,		/* size of block to access */
	      struct cache_blk_t *blk,	/* ptr to block in upper level */
	      tick_t now)		/* time of access */
...
"                -dtlb dtlb:128:4096:32:r\n"
  ...
  //******************************************************************************
  opt_reg_string(odb, "-cache:v",
		 "victim cache config, i.e., {<config>|none}",
		 &vcache_opt, "v:64:32:8:l",
		 /* print */TRUE, NULL);

  opt_reg_int(odb, "-cache:vlat",
	      "victim cache hit latency (in cycles)",
	      &vcache_lat, /* default */6,
	      /* print */TRUE, /* format */NULL);
  //******************************************************************************
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
	...
	fatal("bad l1 D-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>");
      cache_dl1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
			       ...
			       dl1_access_fn, /* hit lat */cache_dl1_lat,4);

      //*********************************************************************
        /* is the victim cache defined? */
        if (!mystricmp(vcache_opt, "none"))
  	{
      	  vcache = NULL;

        /* the level 2 D-cache cannot be defined */
              if (strcmp(cache_dl2_opt, "none"))
        	fatal("the victim cache must defined if the l2 cache is defined");
              cache_dl2 = NULL;
  	}
        else //victim cache is defined
  	{
  	  if (sscanf(vcache_opt, "%[^:]:%d:%d:%d:%c",
  		     name, &nsets, &bsize, &assoc, &c) != 5)
  	    fatal("bad vcache parms: "
  		  "<name>:<nsets>:<bsize>:<assoc>:<repl>");
  	  vcache = cache_create(name, nsets, bsize, /* balloc */FALSE,
  				   /* usize */0, assoc, cache_char2policy(c),
  				   v_access_fn, /* hit latency */0,0);
  ...
  if (!mystricmp(cache_il1_opt, "none"))
	...
	fatal("bad l1 I-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>");
      cache_il1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
			       /* usize */0, assoc, cache_char2policy(c),
			       il1_access_fn, /* hit lat */cache_il1_lat,4);
      ...
      else
	{
	  ...
	  cache_il2 = cache_create(name, nsets, bsize, /* balloc */FALSE,
				   /* usize */0, assoc, cache_char2policy(c),
				   il2_access_fn, /* hit lat */cache_il2_lat,4);
	}
    }
  ...
  //*********************************************************************
  if (vcache_lat < 1)
     fatal("victim cache latency must be greater than zero");
  //*********************************************************************
